...
data_unoptimized = pd.read_csv('../../logs/un-optimized/mean_unopt.txt', sep='\t', header=None, skiprows=72, nrows=36)
data_optimized = pd.read_csv('../../logs/optimized/mean_optimized.txt', sep='\t', header=None, skiprows=72, nrows=36)
...
ax1.plot(subset_unoptimized['R'], subset_unoptimized['memory'], label='Initial Edges 0')
ax1.plot(subset_optimized['R'], subset_optimized['memory'], label='(R / 2) Regular')
configure_plot(ax1, '(Alpha=1.0, L=100)', 'R', 'Memory Usage (KB)', fontsize=18)
...
ax2.plot(subset_unoptimized['R'], subset_unoptimized['memory'], label='Initial Edges 0')
ax2.plot(subset_optimized['R'], subset_optimized['memory'], label='(R / 2) Regular')
configure_plot(ax2, '(Alpha=1.1, L=135)', 'R', 'Memory Usage (KB)', fontsize=18)
...
ax3.plot(subset_unoptimized['R'], subset_unoptimized['memory'], label='Initial Edges 0')
ax3.plot(subset_optimized['R'], subset_optimized['memory'], label='(R / 2) Regular')
configure_plot(ax3, '(Alpha=1.2, L=170)', 'R', 'Memory Usage (KB)', fontsize=18)
ax3.legend(fontsize=18, frameon=False, loc='upper left')

plt.tight_layout()
plt.savefig('optimized_stitch.png')