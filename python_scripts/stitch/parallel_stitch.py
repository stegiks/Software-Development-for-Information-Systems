...
data_unoptimized = pd.read_csv('../../logs/serial/mean_serial.txt', sep='\t', header=None, nrows=36, skiprows=72)
data_optimized = pd.read_csv('../../logs/parallel/mean_parallel.txt', sep='\t', header=None, nrows=36, skiprows=72)
...
ax1.plot(subset_unoptimized['alpha'], subset_unoptimized['index_time'], label='Serial')
ax1.plot(subset_optimized['alpha'], subset_optimized['index_time'], label='Parallel')
configure_plot(ax1, '(L=135, R=64)', 'Alpha', 'Index Time (s)', fontsize=18)
ax1.legend(fontsize=18, frameon=False, loc='upper left')
ax1.set_ylim(0, max(subset_unoptimized['index_time'].max() * 1.2, subset_optimized['index_time'].max()) * 1.2)
...
ax2.plot(subset_unoptimized['R'], subset_unoptimized['index_time'], label='Serial')
ax2.plot(subset_optimized['R'], subset_optimized['index_time'], label='Parallel')
configure_plot(ax2, '(Alpha=1.1, L=135)', 'R', 'Index Time (s)', fontsize=18)
ax2.legend(fontsize=18, frameon=False, loc='upper left')
ax2.set_ylim(0, max(subset_unoptimized['index_time'].max() * 1.3, subset_optimized['index_time'].max()))
...
ax3.plot(subset_unoptimized['L'], subset_unoptimized['index_time'], label='Serial')
ax3.plot(subset_optimized['L'], subset_optimized['index_time'], label='Parallel')
configure_plot(ax3, '(Alpha=1.1, R=64)', 'L', 'Index Time (s)', fontsize=18)
ax3.legend(fontsize=18, frameon=False, loc='upper left')
ax3.set_ylim(0, max(subset_unoptimized['index_time'].max() * 1.2, subset_optimized['index_time'].max()) * 1.2)

plt.tight_layout()
plt.savefig('parallel_stitch.png')